...
<!-- ------------------------------------------------------------------------------------------------------- -->
				...
				if (isset($_SESSION['identity'])) { 
					$sql = mysqli_query($connect, "SELECT * FROM guestcart WHERE user = '{$_SESSION['identity']}' AND status = 1 && date = '{$date}'");
					while ($row = mysqli_fetch_array($sql)) {
						# code...
				 ...
				 <tr id="remove<?php echo $row['id']; ?>">
					...
					<td><span id="txtHint<?php echo $row['id']; ?>"><?php echo $row['price']*$row['quantity']; ?></span></td>
					<!-- Delete Button -->
					<td><span class="text-danger fas fa-times" onclick="remove<?php echo $row['id']; ?>(this.value),update(this.value),total(this.value)" title="Remove from Cart"></span></td>
				 ...
				 <tr>
				 	...
				 	<td colspan="2">Grand Total:</td>
				 	<td>
				 		<span class="" id="total"><?php 
				 		$date = date('d/M/Y');
				 		$total = mysqli_query($connect, "SELECT SUM(quantity*price) AS total FROM guestcart WHERE user = '{$_SESSION['identity']}' && status = 1 && date = '{$date}'");
		while ($rows = mysqli_fetch_array($total)) {
						echo $rows['total'];
					}
				 	 ?>
